...
<?php else : ?>
    ...
    <h3>Your payment was not successful! Please contact us at <a href="mailto:income2017@saske.sk">income2017@saske.sk</a>.</h3>
<?php endif; ?>
  </div>
<?php
  if ($result == "OK") {
    ...
    if (!$error) {
      $sql = "UPDATE Participant SET payment = 'Y', paid = NOW() WHERE payment_id = '" . $paymentid . "'";
    ...
    $message = "Dear " . $name . ",";
    $message .= PAYMENT_OK_PART_1 . $paymentid;
    $message .= PAYMENT_OK_PART_2;
    $message .= PAYMENT_OK_PART_3;

    $mail = mail($email, PAYMENT_SUBJECT, $message,
    ...
    $message .= PAYMENT_ERROR_PART_1 . $paymentid;
    $message .= PAYMENT_ERROR_PART_2;
    $message .= PAYMENT_ERROR_PART_3;
    ...
    if ($email != "") {
      ...
      $mail = mail(FROM, PAYMENT_SUBJECT, $message,
        "Content-Type: text/plain; charset=UTF-8\r\n"
        . "From: INCOME2017 <" . FROM . ">\r\n"
        . "Reply-To: " . FROM . "\r\n"
        . "X-Mailer: PHP/" . phpversion());
